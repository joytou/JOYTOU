...
if(document.getElementById("listgroup"))
{
 $.ajax({
    ...
    eli.innerHTML='<h2>'+data[i].name+'</h2><p class="small">'+data[i].author.login+' published on '+data[i].published_at+'</p><p><strong>Assets</strong><ul><li><a href="'+data[i].tarball_url+'">tar.gz</a></li><li><a href="'+data[i].zipball_url+'">zip</a></li></ul></p><div>'+converter.makeHTML(data[i].body)+'</div>';
